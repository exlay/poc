...
#define LIBPREFIX "/home/vagrant/work/poc/protocols/lib/lib"
...
static int init_exlay(void)
{
	...
	while (fgets(buf, MAX_CFG_LINELEN, fp) != NULL) {
		...
		new_prt->path = malloc(strlen(path)+1);
		memset(new_prt->path, 0, strlen(path)+1);
		memcpy(new_prt->path, path, strlen(path)+1);
...
int *ex_recv_stack_1_svc(int ep, msg buf, int opt, struct svc_req *rqstp)
{
	...
	total_hdr_size = reqired_bufsize(p);
	exd.data = (uint8_t *)malloc(total_hdr_size + buf.msg_len);
	result = read(p->topb->app_r, exd.data, buf.msg_len);
